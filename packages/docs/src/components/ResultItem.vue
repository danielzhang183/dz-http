...
import { isCompact } from '~/logics/state'
  ...
  >
    <template v-if="'url' in item">
      <div :class="[badgeStyle, item.type === 'resource' ? 'badge-square-orange' : 'badge-square-blue']" title="MDN Docs">
        {{ item.type === 'resource' ? 'R' : 'M' }}
      </div>
      <div flex-1>
        <h1>{{ item.type === 'resource' ? 'Resource' : 'MDN' }}: {{ item.title }}</h1>
    ...
    <template v-else-if="item.type === 'guide'">
      <div badge-square-lime :class="badgeStyle" title="Guide">
        G
      </div>
      <div flex-1>
